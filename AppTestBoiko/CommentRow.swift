    ...
    var body: some View {
        ...
        VStack{
            HStack {
                ...
                Text(comment.author).foregroundColor(Color.init(UIColor.label))
                Text(time(time: comment.created)).foregroundColor(Color.init(UIColor.label))
                Text("\u{1F44D}"+String(comment.score)).foregroundColor(Color.init(UIColor.label))
        }
            Spacer()
